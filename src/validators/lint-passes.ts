...
export class LintPassesValidator implements Validator {
  type = 'lint_passes';
  ...
  async validate(
    ...
    const command = (config.command as string | undefined) ?? 'npx eslint .';
    ...
    try {
      ...
      if (!passed) {
        ...
        validationResult.details = `Lint check failed with exit code ${result.exitCode}`;
      }
      ...
      return {
        ...
        target: command,
        passed: false,
        details: `Error running lint check: ${message}`,
        optional: config.optional as boolean | undefined,
      };
    }
  }
