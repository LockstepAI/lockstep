...
export class TypeCheckValidator implements Validator {
  ...
  async validate(
    ...
    const command = (config.command as string | undefined) ?? 'npx tsc --noEmit';
    ...
    try {
      ...
      if (!passed) {
        ...
        validationResult.details = `Type check failed with exit code ${result.exitCode}`;
      }
      ...
      return {
        ...
        target: command,
        passed: false,
        details: `Error running type check: ${message}`,
        optional: config.optional as boolean | undefined,
      };
    }
  }
