...
export type RunnerExecutionMode = 'standard' | 'yolo';
...
export function detectClaudeAuthModes(baseEnv: NodeJS.ProcessEnv = process.env): ClaudeAuthMode[] {
...
export function normalizeRuntimeConfig(config: ClientConfig): RuntimeConfig {
  const runner = config.runner ?? 'codex';
  return {
    runner,
    ...(config.runner_model ? { runnerModel: config.runner_model } : {}),
    judge: config.judge ?? runner,
    ...(config.judge_model ? { judgeModel: config.judge_model } : {}),
    executionMode: config.execution_mode ?? 'standard',
    claudeAuthMode: config.claude_auth_mode ?? 'auto',
  };
}
